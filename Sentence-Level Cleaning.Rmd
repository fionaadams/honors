...
```{r}
dat_word <- readtext("TranscriptTest/*.docx")
dat_word <- dat_word %>% mutate(id = seq.int(nrow(dat_word)))#get numerical ids, so easier to find in future
```


TO ADD: sentence ID column (within each document!!!)
FOR EDA: line graphs of topics vs sentence number
```{r}
#get this as a nested dataframe
library(tidyr)
library(purrr)

testing <- dat_word %>% nest(dat_word = text)
...
for (i in 1:nrow(testing)){
  cleandat <- cleaninterview(testing[[2]][[i]])
...
```{r}
test <- rbindlist(interview_out, fill=FALSE, idcol = "id")

colnames(test)[2] <- "sentences"

dat_word <- dat_word %>% select(id, doc_id)

final <- left_join(test, dat_word, by="id")
#maybe: map doc number back to the document name in dat_word using dplyr::left_join
```
...
corpseg1 <- corpus_segment(corpus, pattern = "##*")
```