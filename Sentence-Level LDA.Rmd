...
tidy_skipgrams <- unclean_sentences %>%
    unnest_tokens(ngram, sentences, token = "ngrams", n=8) %>% #sets sliding window of 8
...
#test: show what this does
new$sentences[9]
lemmatize(new$sentences[9])

# for(i in 1:nrow(new)) {
#   new$sentences[[i]] <- lemmatize(new$sentences[[i]])
# }
...
new_dtm = itoken(new$sentences[1000:2116], tolower, word_tokenizer, ids = new$ID[1000:2116]) %>% 
