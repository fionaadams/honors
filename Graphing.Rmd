...
```{r}
# probability of a topic being in a quadrant
# how to do this? count of topic in each quadrant
# divide by count of topic overall
# get probabilities, map these on y axis?
...
#what do we want?
quantile_probs %>% 
  ggplot(aes(x=quantile, y=p)) + geom_jitter()

long_new %>% 
  filter(id==1) %>%
  ggplot(aes(x=quantile, y=value)) + geom_jitter()
