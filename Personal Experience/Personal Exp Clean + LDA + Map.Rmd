...
s <- udpipe_annotate(udmodel_english, transcripts_personal$text)
x <- data.frame(s)
```
...
This csv doesn't work well because there are commas in the interview, so writing to csv gets all jumbled!
```{r}
write.csv(full_interviews_together, "personalexp_mapped_interviews.csv")
```

So, this writes it to a table! Future steps is to make this into a for loop that writes out a bunch of .txt files all named after the doc_id. Some good info here: https://stackoverflow.com/questions/7033699/write-many-files-in-a-for-loop
```{r}
